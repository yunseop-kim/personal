...
exports.listWithKeyword = function (keyword, callback){
	var sql_cnt = "select count(1) cnt from reviewListView where reviewIsDeleted=0 and instr(productName, '"+ keyword +"') > 0 or instr(productBrand, '" + keyword + "') > 0 or instr(productCategory1, '"+ keyword + "') > 0";

	var sql_list = "select reviewNum, productName, memberId, memberName, productBrand, likeFlagCnt, reviewReplyCnt, reviewHit, " +
	"reviewThumbnail from reviewListView where reviewIsDeleted=0 and ( instr(productName, '"+ keyword +"') > 0 or instr(productBrand, '" + keyword + "') > 0 or instr(productCategory1, '"+ keyword + "') > 0 ) order by reviewRegdate desc limit ?,?";
...
exports.productWrite = "INSERT INTO product (productBrand, productCategory1, productCategory2, productName, productStartAge, productEndAge) VALUES (?, ?, ?, ?, ?, ?)";
...
exports.productWirte = "insert into product (productBrand, productCategory1, productCategory2, productName, " +
...
exports.memberListCnt = "select count(1) cnt from member where instr(memberId, ?) > 0";

exports.memberList =
            "select memberId, memberName, DATE_FORMAT(memberRegdate, '%Y-%m-%d %H:%i:%s') " +
