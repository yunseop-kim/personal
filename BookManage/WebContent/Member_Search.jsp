...
String query_list = "select * from idinfo where Name like '%" + Query + "%' order by IDNO desc";
...
<%   
result_list = stmt_list.executeQuery("SELECT * FROM idinfo where Name like '%" + Query + "%' ORDER BY IDNO DESC LIMIT " + startRow + "," + endRow);
...
%>
    ...
    <button onClick="historyBack()">목록으로 돌아가기</button></form>
<%      
		...
		if( startPageNum - pageGroup > 0 )
			...
			out.println("<a href=./Book_Member_Search.jsp?pageNo=" + (startPageNum - 1) + ">[prev]</a>&nbsp;" );
		for(int i=startPageNum;i<endPageNum;i++) {
			if( i <= totalPage) {
				if( i != currentPage )
					out.println("<a href=./Member_Search.jsp?pageNo=" + i + ">[" + i + "]</a>&nbsp;" );
				else
					out.println("<b>[" + i + "]</b>&nbsp;" );
			}
		}
		if( endPageNum < totalPage ) {
			out.println("<a href=./Member_Search.jsp?pageNo=" + endPageNum + ">[next]</a>&nbsp;" );
		}
