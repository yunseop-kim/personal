...
String query_list = "select * from bookinfo order by BOOKNO desc";
...
%>
<h1>도서 리스트</h1>                        <!-- 헤드라인 글씨를 표현하는 태그입니다. -->
<form method="get">
...
<%   
result_list = stmt_list.executeQuery("SELECT * FROM bookinfo ORDER BY BOOKNO DESC LIMIT " + startRow + "," + endRow);
        while(result_list.next()){
            ...
            out.print("<td>" + boolRent(result_list.getString(12)) + "</td>");
...
//총 페이지 수 계산.
		...
		if( startPageNum - pageGroup > 0 )
			...
			out.println("<a href=./Book_PageViewerRenewaler.jsp?pageNo=" + (startPageNum - 1) + ">[prev]</a>&nbsp;" );
		for(int i=startPageNum;i<endPageNum;i++) {
			if( i <= totalPage) {
				if( i != currentPage )
					out.println("<a href=./Book_PageViewerRenewal.jsp?pageNo=" + i + ">[" + i + "]</a>&nbsp;" );
				else
					out.println("<b>[" + i + "]</b>&nbsp;" );
			}
		}
		if( endPageNum < totalPage ) {
			out.println("<a href=./Book_PageViewerRenewal.jsp?pageNo=" + endPageNum + ">[next]</a>&nbsp;" );
		}
		%>
	<br>
	<button formaction="Book_Register.jsp">등록</button>
    <button formaction="Book_Modify.jsp">수정</button>
    <button formaction="Book_Delete.jsp">삭제</button>
    <INPUT TYPE=TEXT NAME=QUERY><button TYPE=SUBMIT formaction="Book_Search.jsp">검색</button></form>	
<%
        conn.close();
